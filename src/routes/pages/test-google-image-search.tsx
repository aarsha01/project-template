...
export function TestGoogleImageSearch() {
  ...
  const handleSearch = async () => {
    ...
    const response = await fetch(
      `${process.env.BACKEND_API_HOST}/test/google-image-search`,
      {
        method: "POST",
        headers: {
    ...
    setImageItems(data.searchResult.items);
  };

  return (
    <div className="w-full max-w-[900px] mx-auto flex flex-col items-center px-10">
      <h1 className="mt-10 text-3xl">Test google image search</h1>
      <textarea
        ref={textAreaRef}
      ...
      <div className="flex flex-wrap gap-4">
        {imageItems.map((imageItem) => (
          <div
            ...
            style={{
              backgroundImage: `url(${imageItem.image.thumbnailLink})`,
